$(document).ready(function() {
  $('.insert-before-btn').click(function() {
    var pageWrapper = $(this).closest('.page-wrapper'); // Obtén el contenedor de la imagen actual
  ...
  const textElements = document.querySelectorAll(".page-text.large");

  textElements.forEach((textElement) => {
    const maxTotalLength = 26;

    textElement.addEventListener("input", function () {
      let currentText = textElement.innerText;
      if (currentText.length > maxTotalLength) {
        currentText = currentText.substring(0, maxTotalLength);
        textElement.innerText = currentText;
        // Muestra una alerta con SweetAlert cuando se excede el límite
        swal({
          icon: "error",
          title: "Límite de caracteres alcanzado",
          text: "Ya no puedes escribir más de 26 caracteres.",
...
$(document).ready(function() {
  $('.image-group37 img').each(function() {
    $(this).click(function() {
      ...
      // Eliminamos el elemento con id "remove-img" al cambiar la imagen del modal
      $('#remove-img').remove();
    });
  });
});
